...
public:
    ...
    // Copy interleaved mono/stereo samples; the caller retains its buffer.
    // Invalid data throws std::invalid_argument without replacing the old sample.
    bool loadArray(const float* audio, size_t frames, int channels, double sampleRate);
    ...
    // Snapshot of one active voice for visualization: every phase the engine
    // carries, in seconds or normalized units, nothing that needs the buffer.
    struct VoiceState {
        int note;            // MIDI note
        double playhead;     // seconds into the file
        double grainPhase1;  // 0-1
        double grainPhase2;  // 0-1
        double grainPos1;    // seconds into the file
        double grainPos2;    // seconds into the file
        int envState;        // EnvState
        double level;        // amplitude after envelope/decay/fade, before gain
        double direction;    // +1 forward, -1 reverse
    };
    // Fill up to `max` states; returns the number written.
    int getVoiceStates(VoiceState* out, int max) const;
