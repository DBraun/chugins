...
SamplerVoice::SamplerVoice()
    : playhead(0.0), velocity(1.0), pingpongDirection(1.0), stopPosition(0.0), sliceStart(0.0), sliceEnd(0.0),
      grainPhase1(0.0), grainPhase2(0.5), grainPos1(0.0), grainPos2(0.0), envValue(0.0), envState(ENV_IDLE),
      holdCounter(0), releaseRequested(false), ageSamples(0), currentLevel(0.0) {}
...
// File loading
//-----------------------------------------------------------------------------
bool Sampler::loadArray(const float* audio, size_t frames, int channels, double sampleRate) {
    if (!isFinite(sampleRate) || sampleRate <= 0.0 || channels < 1 || channels > 2 ||
        frames > static_cast<size_t>(std::numeric_limits<int>::max()) / channels ||
        frames > SIZE_MAX / channels / sizeof(float) || (frames > 0 && !audio)) {
        throw std::invalid_argument("invalid audio shape or sample rate");
    }
    const size_t count = frames * channels;
    auto buffer = std::make_unique<float[]>(count);
    for (size_t i = 0; i < count; ++i) {
        if (!std::isfinite(audio[i])) throw std::invalid_argument("audio contains NaN or inf");
        buffer[i] = audio[i];
    }
    allNotesOff();
    clearBuffer();
    ...
    m_loopEnd = 1.0;
    m_startMarker = 0.0;
    m_endMarker = static_cast<double>(frames) / sampleRate;
    return true;
}
...
double Sampler::confineGrainPos(const SamplerVoice* voice, double pos, double regionLo,
...
int Sampler::getVoiceStates(VoiceState* out, int max) const {
    int n = 0;
    const double fs = m_fileSampleRate > 0.0 ? m_fileSampleRate : m_systemSampleRate;
    m_voiceManager.forEachActiveVoice([&](const SamplerVoice* v) {
        if (n >= max) return;
        VoiceState& s = out[n++];
        s.note = v->note;
        s.playhead = v->playhead / fs;
        s.grainPhase1 = v->grainPhase1;
        s.grainPhase2 = v->grainPhase2;
        s.grainPos1 = v->grainPos1 / fs;
        s.grainPos2 = v->grainPos2 / fs;
        s.envState = static_cast<int>(v->envState);
        s.level = v->currentLevel;
        s.direction = v->pingpongDirection;
    });
    return n;
}
...
Sampler::StereoSample Sampler::generateGrain(const SamplerVoice* voice, double grainPhase, double grainPos,
    ...
    StereoSample sample =
        readSample(confineGrainPos(voice, grainPos, regionLo, regionHi), regionLo, regionHi);
